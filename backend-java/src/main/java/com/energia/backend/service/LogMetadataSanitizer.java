...
public class LogMetadataSanitizer {
    ...
    private static final Pattern PHONE_PATTERN = Pattern.compile("\\b(\\+?\\d{1,3}[\\s-]?)?(\\(?\\d{2,3}\\)?[\\s-]?)?\\d{4,5}[\\s-]?\\d{4}\\b");
    private static final Pattern PASSWORD_PATTERN = Pattern.compile("(?i)\\b(senha|password|pwd)\\b['\"]?\\s*[:=]\\s*['\"]?[^,'\"} ]+");
    private static final Pattern TOKEN_PATTERN = Pattern.compile("(?i)\\b(token|jwt|bearer)\\b['\"]?\\s*[:=]\\s*['\"]?[^,'\"} ]+");
    private static final Pattern AUTH_HEADER_PATTERN = Pattern.compile("(?i)authorization['\"]?\\s*[:=]\\s*['\"]?[^,'\"} ]+");
    private static final Pattern COOKIE_PATTERN = Pattern.compile("(?i)cookie['\"]?\\s*[:=]\\s*['\"]?[^,'\"} ]+");
    private static final Pattern SECRET_PATTERN = Pattern.compile("(?i)secret|credential|api[_-]?key|chave[_-]?secreta");
    ...
    private String sanitizeValue(String key, Object value) {
        ...
        if (key.toLowerCase().contains("payload") || key.toLowerCase().contains("request") || key.toLowerCase().contains("response")) {
            return MASK;
        }
